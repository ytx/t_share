...
1. **右クリックメニューの実装**
   ...
   - 「定型文を作成」: 選択テキストから定型文作成モーダルを表示
   - 「上のエディタへ移動」: 選択テキストを切り取り、上部エディタに挿入
   - Material-UI MenuコンポーネントによるUI実装

2. **定型文作成機能の統合**
   - 選択テキストを`initialContent`として定型文作成モーダルに渡す
   - 上部エディタと同じモーダルUIで統一
   - タイトル・タグ・シーン・公開設定の編集が可能
   - 作成後は定型文検索結果に即座反映

3. **上のエディタへ移動機能**
   - 下段エディタで選択したテキストを切り取り
   - 上段エディタ（DocumentEditor）のカーソル位置に挿入
   - MarkdownEditorの`insertText`メソッドを活用
   - 挿入後、カーソルを挿入テキストの末尾に移動

4. **Undo/Redo対応の修正**
   - `useRef`による最新content状態の保持（contentRef）
   - autoSave/flush関数での`contentRef.current`参照
   - Ctrl-Z（Undo）、カット、ペースト操作が正しく動作
   - タイマーによる古いcontent上書き問題の解決

5. **技術的実装詳細**
   - `onContextMenuCapture`でイベントキャプチャフェーズ処理
   - MarkdownEditorの既存コンテキストメニューとの競合回避
   - `event.preventDefault()`と`event.stopPropagation()`の適切な使用
   - DocumentEditorのforwardRefによるinsertTextメソッド公開
...
2. **個人メモ機能（プロジェクト別サーバーストレージ）**
   - タイトル「メモ（自分用）」の文書をプロジェクトごとに自動作成
   - LocalStorageからサーバーストレージに完全移行
   - プロジェクトごとにユーザーごと1つのメモ文書（作成者のみアクセス可能）
   - プロジェクト未選択時は全体メモ（projectId=null）
   - 複数デバイス間での同期が可能に
   - タブ名: 「メモ（自分用）」
...
4. **バックエンドAPI拡張**
   ...
   - `GET /api/documents/personal-memo?projectId={id}`: プロジェクト別個人メモ取得・作成
   - 特殊文書のアクセス制御実装
